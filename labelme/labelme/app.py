...
class LabelQLineEdit(QtWidgets.QPlainTextEdit):
    ...
    def keyPressEvent(self, e):
        if not self.block and e.key() in [QtCore.Qt.Key_Up, QtCore.Qt.Key_Down]:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        # List of novel labels to add to txt
        self.novel_label   = []
        self.novel_context = []
        self.novel_state   = []
        self.novel_person  = []
        ...
        self.free_dock = self.free_widget = None
        self.free_dock = QtWidgets.QDockWidget(self.tr('Description'), self)
        self.free_dock.setObjectName('Description')
        self.free_widget = LabelQLineEdit(block=True)
        self.free_dock.setWidget(self.free_widget)
        #self.free_widget.itemChanged.connect(self.setDirty)
        ...
        # Add Context List
        def addOptionList(oList, option="context"):
            oList.setToolTip(self.tr(
                "Select label to start annotating for it. "
                "Press 'Esc' to deselect."))
            if self._config[option]:
                for label in self._config[option]:
                    item = oList.createItemFromLabel(label)
                    oList.addItem(item)
                    oList.setItemLabel(item, label, [0,0,0])
            dock = QtWidgets.QDockWidget(self.tr(f'{option} List'), self)
            dock.setObjectName(f'{option} List')
            dock.setWidget(oList)
            return dock

        self.uniqContextList = UniqueLabelQListWidget()
        self.context_dock    = addOptionList(self.uniqContextList, "context")

        self.uniqStateList = UniqueLabelQListWidget()
        self.state_dock    = addOptionList(self.uniqStateList, "state")

        self.uniqPersonList = UniqueLabelQListWidget()
        self.person_dock    = addOptionList(self.uniqPersonList, "person")

        self.uniqOrientList = UniqueLabelQListWidget()
        self.orient_dock    = addOptionList(self.uniqOrientList, "orient")

        self.uniqPhraseList = UniqueLabelQListWidget()
        self.phrase_dock    = addOptionList(self.uniqPhraseList, "phrase")
        ...
        for dock in ['flag_dock','free_dock', 'label_dock','context_dock', 'state_dock', 'person_dock', 'orient_dock', 'phrase_dock', 'shape_dock', 'file_dock']:
        ...
        self.addDockWidget(Qt.RightDockWidgetArea, self.free_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.label_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.context_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.state_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.person_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.orient_dock)
        self.addDockWidget(Qt.RightDockWidgetArea, self.phrase_dock)
        ...
        grabcut = action(
            text = self.tr('&Apply_cut'),
            slot=lambda: self.toggleDrawMode(False, createMode='grab'),
            shortcut= shortcuts['Apply_cut'],
            icon='Grab',
            tip = self.tr('Apply grab cut on the image'),
            enabled=False)
        ...
        utils.addActions(
            ...
            (
                ...
                self.free_dock.toggleViewAction(), # TOGGGG
                self.label_dock.toggleViewAction(),
                self.shape_dock.toggleViewAction(),
                self.file_dock.toggleViewAction(),
                self.context_dock.toggleViewAction(),
                self.state_dock.toggleViewAction(),
                self.person_dock.toggleViewAction(),
                self.orient_dock.toggleViewAction(),
        ...
        #TESTING
        try:
            self.labelDialog.setWinSize(self.settings.value('label/size', []))
        except ValueError:
            print("Initializing default size")
    ...
    def populateModeActions(self):
        ...
        actions = (
            ...
            self.actions.grabcut, ################################################################### SOUAYB 11.02
    ...
    def editLabel(self, item=None):
        ...
        for idx,poly in enumerate(polygons):
            #contained = True
            #p1 = polyshape([[point.x(),point.y()] for point in points])
            #p2 = polyshape(poly)

            #for point in points:
            #    if not self.ray_tracing_method(point.x(),point.y(),poly):
            #        contained = False
            #        break
            #if contained:
            #if not p1==p2 and p1.intersects(p2):
            if poly_ids[idx] is not None:
                levels.append(poly_labels[idx]+"_"+str(poly_ids[idx]))
            else:
                levels.append(poly_labels[idx])
        if len(levels)>0:
        ...
        if not self.uniqLabelList.findItemsByLabel(shape.label):
            ...
            self.novel_label.append(shape.label)

        def addUniqHistory(oList,option=shape.context,novel=self.novel_context):
            if not oList.findItemsByLabel(option):
                item = oList.createItemFromLabel(option)
                oList.addItem(item)
                rgb = [0 ,  0  , 0]
                oList.setItemLabel(item, option, rgb)
                novel.append(option)

        addUniqHistory(self.uniqContextList,shape.context,self.novel_context)
        addUniqHistory(self.uniqStateList,shape.state,self.novel_state)
        addUniqHistory(self.uniqPersonList,shape.person,self.novel_person)
        addUniqHistory(self.uniqOrientList,shape.orient,self.novel_orient)
        addUniqHistory(self.uniqPhraseList,shape.phrase,self.novel_phrase)

        self.labelDialog.addLabelHistory(shape.label)

        if shape.context is not None:
            self.labelDialog.addHistory(shape.context,self.labelDialog.contextList)
        if shape.state is not None:
            self.labelDialog.addHistory(shape.state,self.labelDialog.stateList)
        if shape.person is not None:
            self.labelDialog.addHistory(shape.person,self.labelDialog.personList)
        if shape.orient is not None:
            self.labelDialog.addHistory(shape.orient,self.labelDialog.orientList)
        if shape.phrase is not None:
            self.labelDialog.addHistory(shape.phrase,self.labelDialog.phraseList)
    ...
    def addLabel(self, shape):
        ...
        def addUniq(oList,option,novel):
            if not oList.findItemsByLabel(option):
                item = oList.createItemFromLabel(option)
                oList.addItem(item)
                rgb = [0 ,  0  , 0]
        ...
        addUniq(self.uniqContextList, shape.context, self.novel_context)
        addUniq(self.uniqStateList, shape.state, self.novel_state)
        addUniq(self.uniqPersonList, shape.person, self.novel_person)
        addUniq(self.uniqOrientList, shape.orient, self.novel_orient)
        addUniq(self.uniqPhraseList, shape.phrase, self.novel_phrase)
    ...
    def loadFree(self, free):
        self.free_widget.clear()
        try:
            self.free_widget.setPlainText(free)
        except TypeError:
            self.free_widget.setPlainText("")
    ...
    def saveLabels(self, filename):
        ...
        def format_shape(s):
            ...
            data.update(dict(
                ...
                #context=s.context.encode('utf-8') if PY2 else s.context,
        ...
        free_text = self.free_widget.toPlainText()
    ...
    def newShape(self):
        ...
        for idx,poly in enumerate(polygons):
            ...
            #p2 = polyshape(poly)
            #for point in points:
            #    if self.ray_tracing_method(point.x(),point.y(),poly):
            #        contained = True
            #        break
            #if contained:
            #if p1.intersects(p2):
    ...
    def loadFile(self, filename=None):
        ...
        if QtCore.QFile.exists(label_file) and \
            ...
            try:
                ...
                # Update group ids in labeldialog
                self.labelDialog.group_ids = self.labelFile.group_ids
            except LabelFileError as e:
                self.errorMessage(
                    self.tr('Error opening file'),
                    self.tr(
        ...
        free = ""
        if self.labelFile:
            if self.labelFile.free is not None:
                free = self.labelFile.free
        print(free)
        self.loadFree(free)
    ...
    # Writes new labels to save txt
    def update_txts(self):
        # Save new labels/context

        with open("./logs/labels.txt","a") as file:
            for label in self.novel_label:
                if type(label)==str:
                    file.write("\n"+label)
            file.close()

        with open("./logs/context.txt","a") as file:
            for context in self.novel_context:
                if type(context)==str:
                    file.write("\n"+context)
            file.close()

        with open("./logs/state.txt","a") as file:
            for state in self.novel_state:
                if type(state)==str:
                    file.write("\n"+state)
            file.close()

        with open("./logs/person.txt","a") as file:
            for person in self.novel_person:
                if type(person)==str:
                    file.write("\n"+person)
            file.close()

        with open("./logs/orient.txt","a") as file:
            for orient in self.novel_orient:
                if type(orient)==str:
                    file.write("\n"+orient)
            file.close()

        with open("./logs/phrase.txt","a") as file:
            for phrase in self.novel_phrase:
                if type(phrase)==str:
                    file.write("\n"+phrase)
            file.close()
    ...
    def closeEvent(self, event):
        ...
        # Save labeldialog size
        self.settings.setValue('label/size', (self.labelDialog.height,self.labelDialog.width))
